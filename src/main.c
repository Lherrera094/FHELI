...
//{{{
    ...
    // set-up grid
    control_init(  gridCfg, beamCfg, saveDCfg, antDetect, helicAnt );
    ...
    init_antennaInjection( gridCfg, beamCfg, helicAnt );
...
#pragma omp parallel private(n_threads)
    ...
    for (t_int=0 ; t_int <= T_END ; ++t_int) {
        ...
        control_antennaInjection(  gridCfg, beamCfg, helicAnt, t_int, EB_WAVE, EB_WAVE_ref ); //beam injection into grid
